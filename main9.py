...
def drawField(field):
    for row in range(7):  #0,1,2,3,4,5
                          #0,.,1,.,2
        ...
        if row%2 == 0:
            ...
            for column in range(15):  # will take values 0 (in drawing) -> 0 (in actual field), 1->., 2->1, 3->., 4->2
...
while(True): # True == True / is always true (We can also use while(1))
    ...
    MoveRow = MoveRow #int(input("Please enter the row\n"))       # Convert the row to integer
    if currentField[MoveColumn][MoveRow] == "X" or currentField[MoveColumn][MoveRow] == "O":
        MoveRow = MoveRow - 1
    if Player == 1:
